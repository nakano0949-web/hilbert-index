Overview
HilbertIndex2D is a 2D spatial indexing library based on the Hilbert space‑filling curve.  
It provides fast rectangular range queries and k‑nearest neighbor (kNN) search for large 2D datasets.

The Hilbert curve preserves spatial locality more strongly than Z‑order (Morton), meaning that points close in 2D tend to remain close in the 1D Hilbert ordering.  
...
tatsuya、次はこの README をリポジトリに組み込む？  
それとも GitHub のフォルダ構成を一緒に作る？- skipping cells completely outside the rectangle  
- adding entire Hilbert intervals for cells fully inside  
- subdividing partially overlapping cells  
- scanning efficiently using lowerBound and upperBound  
